...
if $0 !~ /(^|\/)gem$/ && (ENV['VHOST'] == 'true' || ["./frozen_gems.txt"].any? {|frozen_file| File.exists?(frozen_file)})
  ...
  class Gem::SourceIndex
    ...
    def spec_dirs
      (@parent ? [@spec_dir] + parent.spec_dirs : [spec_dir]).compact
    end
    ...
    def each(&block) # :yields: gem.full_name, gem
      @gems.for(spec_dir).each(&block) if spec_dir
  ...
  module Gem
    ...
    def self.activate(agem, *version_requirements)
      ...
      puts "activate #{agem} -> no matches!" if matches.empty?
    ...
    def self.path
      ...
      unless @gem_path then
        ...
        # Automatically include ./gems in the path, if it exists - as the highest priority gem location.
        paths.unshift("#{Dir.pwd}/gems") if File.directory?("#{Dir.pwd}/gems") && File.directory?("#{Dir.pwd}/gems/specifications")
        # Would like to support Rails' vendor/gems thing, but gems are simply unpacked there instead of installed...
        #  - need to figure out how this works.
        # Pulled vendor_gem_source_index and supporting files from Rails, hopefully that's all we need.
        require 'rubygems/defaults/support/rails_gem_dependency' if File.directory?("#{Dir.pwd}/vendor/gems")
